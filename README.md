...
1. 尚未在真实大型项目中大规模测试
2. 异构审计降低但不消除错误——GPT-4o 和 Claude 在异步并发等领域盲区重合
3. 自动生成的测试仅覆盖 happy path，边界情况需人工补充
4. 记忆衰减参数是初始值，需通过实验校准
5. 代码分析器不处理 `importlib` 动态导入和条件导入
6. 扫描模式的"IO与业务逻辑混杂"检测基于关键词匹配，存在误报可能
7. 地图模式的层级分类基于依赖图的中位数分割，小型项目（<10 文件）分层可能不准确
